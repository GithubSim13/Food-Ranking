...
client/src/
  ...
  components/
    ...
    common/
      ...
      pageStyles.ts     # shared inline style objects: kickerStyle (0.7rem), pageTitleStyle, smallPrimaryBtnStyle, smallSecondaryBtnStyle, smallDeleteBtnStyle (all 0.78rem, 0.3rem vertical padding, 150ms transition)
    ...
    home/
      ...
      ReigningChampionCard.tsx  # Best of the Month card; highest-rated entry with a review this calendar month; falls back to all-time highest-rated; gold if starred, accent/purple if not
    analytics/
      AnalyticsPage.tsx # /analytics — stats and insights page; page resets `<main>` scroll position to top on mount (rootRef → closest('main').scrollTop = 0, since AppShell's <main> persists across navigation); sections: Rating Distribution donut chart (avg rating shown as center label in donut hole, var(--accent), 1.8rem; counts up 0→value over ~800ms ease-out when scrolled into view via useCountUp + useInViewOnce IntersectionObserver, fires once; donut hole ratio 0.78), Starred Ratio donut chart (starred count shown as center label in donut hole, var(--gold), 1.8rem; same scroll-triggered count-up, whole numbers), Top Categories by Avg Rating (min 2 rated entries; bars use red→yellow→green spectrum gradient clipped by fill width via backgroundSize, numeric label scoreColor; bars animate width 0→final on scroll-into-view, 500ms ease-out, 60ms stagger, fires once), Best Restaurants by Avg Rating (min 2 rated entries; renders RankRow rows, no bars), Most Visited Stores (top 10 by entry count, Nx format), Most Logged Categories (top 10 by entry count, Nx format), Category Insights card (Best Spot per Category + Score Breakdown by Category merged into one Card with kicker "CATEGORY INSIGHTS"; shared single-line scrollable pill strip from breakdownCategories alphabetically sorted; pill strip: flexWrap nowrap, overflowX auto, gap 6px, padding 2px 0, hide-scrollbar class; left/right gradient fade overlays (var(--paper-2)→transparent, 32px wide, position absolute, opacity driven by catPillScroll state via scroll listener + ResizeObserver on pillScrollRef); active pill auto-scrolls into view on effectiveCategory change via pillRefsMap Map ref — scrolls the pill strip **horizontally only** (container.scrollBy on pillScrollRef computed from bounding rects), NOT scrollIntoView, which would also scroll <main> vertically and yank the page on mount; effectiveCategory = activeCategory ?? defaultBreakdownCat; Best Spot sub-section shows restaurants with ≥2 entries in selected category sorted by avg rating desc; Score Breakdown sub-section (subtitle "How it actually tastes, costs, and holds up.") shows three T/V/C horizontal bars with full spectrum gradient clipped by fill width and 400ms width transition, value labels scoreColor; sections separated by var(--line) hr; empty state per sub-section if data insufficient), Rating Trajectory movers (kicker MOMENTUM, subtitle "Foods that have gotten better or worse over repeat visits."; entries with 2+ reviews, filter: improved/declined/all), Logging Activity Calendar (kicker CONSISTENCY, subtitle "Every cell is a day you ate something worth logging."; GitHub-style heatmap of review dates; cells with count > 0 play a scale 1→1.15→1 pulse on scroll-into-view, staggered 20ms per column, fires once; year picker pills inline with "Logging Activity" heading in flex justify-content space-between header row; 53×7 grid cells 11×11px with 2px gap, coloured via color-mix(in srgb, var(--accent) 25/55/100%, transparent) by review count 0/1/2–3/4+; month labels above grid using absolute positioning at col×13px; Mon/Wed/Fri day-of-week labels; hover tooltip position:fixed with date + review count; horizontally scrollable wrapper), Consistency vs Taste scatter (plain SVG, X=Consistency rating3, Y=Taste rating1; quadrant dividers at x=5/y=5 with corner labels: Reliable & Tasty / Tasty but Inconsistent / Consistent but Bland / Avoid; dots r=5 fill var(--accent)/var(--gold) for starred, hover r=7; fixed tooltip with food/restaurant/category/T+C/overall; click opens entry modal; ResizeObserver for responsive width; Card has minWidth: 0, container div has overflow: hidden), Price vs Rating scatter (plain SVG, X=Price ₱ with maxPrice rounded up to nearest 50, Y=Rating 0–10; same dot/hover/click pattern as Consistency vs Taste; uses latest review with both price and overallRating non-null; shows empty state if <3 data points; shares 1fr 1fr grid row with Consistency vs Taste; Card has minWidth: 0, container div has overflow: hidden), Long Time No See (kicker OVERDUE, subtitle "One visit. Never went back. You had one job."; entries with exactly 1 review and non-null date, sorted oldest-visit-first; paginated LTNS_PAGE_SIZE = 10, prev/next buttons, "Showing X–Y of Z" label, row # uses global index ltnsPage × LTNS_PAGE_SIZE + i + 1; table columns: #/Entry/Category/Restaurant/Visited/Days Ago; Days Ago coloured: <180d var(--ink-mute), 180–365d var(--gold), >365d var(--badge-never-again); "Xd" Space Mono; local date parsing split on '-' not new Date()), Rating by Country (sortable table grouped by Entry.flag, null flag = "🏠 Home" row; FlagImage + full country name from countryList.ts (fallback raw code); columns: #/Country/Entries/Avg Rating (scoreColor label + inline proportional bar using red→yellow→green spectrum gradient clipped by fill width; bars animate width 0→final on scroll-into-view, 500ms ease-out, 60ms stagger, fires once)/Best Entry (accent clickable span opens entry modal); default sort avg rating desc; sort cycles asc/desc per column), Underrated Gems (kicker HIDDEN, subtitle "High scores, one visit. You're sleeping on these."; entries with latestRating ≥ 8.0 and reviews.length === 1; responsive 3/2/1 card grid matching Starred Picks; card: flag, food name truncated, restaurant muted, category pill accent-wash/accent-ink, rating headline-sized 2rem accent mono weight 800 bottom-right, "1 visit" + muted "↗ revisit?" nudge bottom-left, tryAgain blue dot via EntryFlagBadges, hover border var(--accent) transition 150ms; cards fade-in + slide-up (gemIn keyframes, 400ms ease-out, 80ms stagger) on scroll-into-view, re-triggers per page since cards keyed by entry.id; click opens entry modal; paginated GEMS_PAGE_SIZE = 9, prev/next buttons, "Showing X–Y of Z" label), Starred Picks grid (paginated, STARRED_PAGE_SIZE = 9, prev/next buttons, "Showing X–Y of Z" label); page subtitle reads "{entries.length} foods logged. Here's what they say about you." (entries.length dynamic); all computed client-side from ['entries'] query. Scroll-triggered animation helpers (module-level): `useInViewOnce` (callback-ref IntersectionObserver, fires once, works for late-mounted elements after the query resolves) and `useCountUp` (rAF cubic ease-out counter). All rating bars across the page use the red→yellow→green spectrum gradient (matching RatingInput) clipped to the fill width via `backgroundSize: ${1000/avg}% 100%` so the visible slice reflects the score, with numeric labels coloured by scoreColor()
    entries/
  ...
  utils.ts              # shared helpers: sortReviewsByDateDesc, latestRating, latestRatedReview, latestPrice, scoreColor, formatReviewDate, getLocalDateString() (returns local YYYY-MM-DD string, UTC+8 safe), autoResize() (textarea height-to-content helper)
  ...
  main.tsx              # QueryClientProvider + BrowserRouter + ToastProvider + ToastContainer; QueryClient has defaultOptions: { queries: { staleTime: 5 * 60 * 1000 } }
```

### Key behaviours

- **Vite proxy**: `/api` → `http://localhost:3000`
- **staleTime**: set to 5 minutes globally on QueryClient — window-focus refetches do not trigger unnecessary re-computation
- **Design system**: Ube Midnight dark palette — CSS variables (`--paper`, `--paper-2`, `--surface`, `--ink`, `--ink-mute`, `--line`, `--accent`, `--accent-wash`, `--accent-ink`, `--gold`, `--gold-wash`, `--line-soft`, `--danger`, `--danger-wash`, `--danger-border`, `--accent-light`, `--accent-soft`, `--toast-success-bg/-border/-ink`, `--toast-error-bg/-border/-ink`, etc.) in `index.css`. Fonts via `--font-display` (Bricolage Grotesque), `--font-body` (Hanken Grotesk), `--font-mono` (Space Mono) — never hardcode font family strings. All modals, dropdowns, inputs, and buttons use CSS variables — no hardcoded colors anywhere. `index.css` also defines `.hall-title-fame` (green glow text-shadow for Hall of Fame backdrop title), `.hall-title-shame` (red glow text-shadow for Hall of Shame backdrop title), a global `:focus-visible` rule (2px `var(--accent)` outline on buttons/inputs/textareas/selects/links), `.pill:hover` (accent border + ink text on hover), and `.icon-btn:hover` / `.icon-btn-danger:hover` (background highlight for icon buttons).
- **UI consistency standards** (apply to all new UI): form inputs `padding: 0.45rem 0.65rem`; form labels `fontSize: 0.85rem`, `marginBottom: 0.25rem`; primary/submit buttons `padding: 0.5rem 1rem`; small buttons (pageStyles.ts) `fontSize: 0.78rem`, `0.3rem` vertical padding; kicker `fontSize: 0.7rem`; pills and buttons get `transition: 'all 150ms ease'`; pill buttons take `className="pill"` and icon buttons take `className="icon-btn"` (or `icon-btn-danger`) for CSS hover states; submit/mutation buttons show `opacity: 0.6` while pending; flag images render at the FlagImage default `1.25em` width (no per-usage width overrides); icon/flag-to-label gap is `0.4rem`.
...
- **Move Entries (Rankings page)**: sticky bar "Move Entries" button (enabled once a category has scrolled into view via active category detection). Clicking locks the source category and enters move mode — scope pills are replaced with a confirmation row (← source label, destination name input auto-focused, "Move N entries" confirm button, Cancel). Entries outside the source category are dimmed and non-interactive. Entries inside the source category are clickable to toggle selection (accent-wash + accent border when selected; wiggle animation on first selection). Source category header turns `var(--accent)`. Warning line shown below sticky bar when all entries in the source category are selected ("Moving all entries will delete the category"). Confirm validates same-category destination, calls `PATCH /api/entries/bulk-move`, invalidates `['rankings']`, `['entries']`, `['categories']`, exits move mode. Move mode and edit-rankings mode are mutually exclusive (entering move mode exits edit mode). Note: active category detection uses `[data-category]` elements with `getBoundingClientRect().top <= --search-bar-height + 32px`; because category headers are `position: sticky`, the user needs to scroll slightly past a header for it to register as active.
...
| **Best of the Month** | Highest-rated entry (latestRating desc) whose latest non-null review.date falls in the current calendar month (split on '-', no `new Date()`); tiebreak: latestRating desc, then reviews.length desc. Falls back to all-time highest-rated if no entries have a review this month. Any entry qualifies (not restricted to starred). Card label reads "Best of the Month"; subtitle reads "Best of {MonthName}" (e.g. "Best of June"). Gold card background if winning entry is starred, accent/purple if not. |
