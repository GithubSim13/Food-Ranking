...
- **Design system**: Ube Midnight dark palette — CSS variables (`--paper`, `--paper-2`, `--surface`, `--ink`, `--ink-mute`, `--line`, `--accent`, `--gold`, etc.) in `index.css`. Fonts: Bricolage Grotesque (display), Hanken Grotesk (body), Space Mono (mono). All modals, dropdowns, inputs, and buttons use CSS variables — no hardcoded light colors anywhere. `index.css` also defines `.hall-title-fame` (green glow text-shadow for Hall of Fame backdrop title) and `.hall-title-shame` (red glow text-shadow for Hall of Shame backdrop title).
...
| **🏆 Hall of Fame** | Top 5 entries by highest overallRating (latest review per entry); full-width upward podium — bars grow upward, `alignItems: 'flex-end'` row, purple color scheme; order left to right: 4, 2, 1, 3, 5; bar heights: #1=65%, #2=50%, #3=40%, #4=28%, #5=18%; label (flag + name + score) above each bar; inside bars 1–3: scaled rank number (no emoji) + category + restaurant + review count; inside bars 4–5: scaled rank number only; rank font sizes: #1=2.2rem, #2=1.7rem, #3=1.35rem, #4=1.05rem, #5=0.9rem, weight 800; clicking any bar opens entry modal via background-location pattern; large backdrop watermark "🏆 Hall of Fame" with `.hall-title-fame` green glow |
| **💀 Hall of Shame** | Bottom 5 rated entries (min one review) sorted by overallRating asc; full-width downward podium directly below Hall of Fame — bars grow downward, `alignItems: 'flex-start'` row, orange color scheme; same 5-column order; bar depths mirror fame heights; label (flag + name + score) below each bar; inside bars 1–3: scaled rank number (no emoji) + category + restaurant + review count; clicking any bar opens entry modal; large in-flow watermark "💀 Hall of Shame" with `.hall-title-shame` red glow below bars |
