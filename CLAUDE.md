...
client/src/
  ...
  utils.ts              # sortReviewsByDateDesc, latestRating, latestRatedReview, latestPrice, scoreColor, scoreColorAlpha (same hue + alpha, for score glows), formatReviewDate, getLocalDateString(), autoResize() — applied on load (useEffect with value dep), onChange, and onPaste (setTimeout 0) across all notes textareas; useInViewOnce(ref) — IntersectionObserver hook (threshold 0.12, viewport root, fires once) for scroll-triggered entrances; useCountUp(target, start, duration?) — eases 0→target once start flips true (AnalyticsPage keeps its own local copy)
...
- **Animations**: shared keyframes + utility classes live at the bottom of `index.css` — `@keyframes fadeSlideUp / fadeIn / scaleIn`; always-on entrance classes `.anim-fade-slide-up`, `.anim-fade-in`, `.anim-scale-in` (run on mount); scroll-triggered variant `.anim-on-view` (starts hidden) + `.is-visible` (plays once in view, paired with `useInViewOnce`); stagger helpers `.anim-delay-1`…`.anim-delay-8`; hover utilities `.hover-lift` (translateY + shadow) and `.hover-scale`. All respect `prefers-reduced-motion: reduce`. CSS variables only — no hardcoded colors. Applied as: staggered card/row entrances (EntryCard/EntryList, CategoriesPage rows), scroll-triggered reveals (HomePage non-podium sections via a local `Reveal` wrapper, RankingsPage sticky category headers via opacity-only to preserve sticky positioning), `.anim-scale-in` keyed on entry id (QuickRatePage card), on the NotFoundPage achievement card (the module's `.toast` keeps its bespoke `toastDrop`/`idleBob` via a `.wrapper .toast` specificity bump so the global utility never clobbers it) and its copy block, `.anim-fade-in` on EntryDetail mount, `.hover-lift` on EntryCard + restaurant cards + Home stat cards. RestaurantsPage cards use `.anim-fade-slide-up` **without** a `.anim-delay-*` stagger — the CSS-columns masonry is DOM-ordered column-first, so per-card stagger grouped by column and looked broken; cards now animate in together and are keyed on `id-sortCol-sortDir` so they remount and replay the entrance on sort change. The bespoke animations in PodiumSection, AnalyticsPage (it keeps its own local `useInViewOnce`/`useCountUp`), Modal, and Toast are independent and untouched.
- **Visual polish (second pass)**: built on the animation utilities above, all CSS-variable-only (`color-mix(in srgb, var(--gold) …%, transparent)` for translucent gold):
  - **Stat count-up**: Home dashboard stat numbers (categories/foods/starred/restaurants) ease 0→value via `useCountUp`, gated by `useInViewOnce` on the stat row. The About callout count also uses `useCountUp` but runs on mount (`start: true`) rather than scroll-gated — it sits at the bottom of the page where the viewport-rooted observer wasn't firing reliably, so it was rendering `0`. Per-row counts on Categories/Restaurants are intentionally left static (too many small numbers to animate tastefully).
  - **Score glow** (`EntryCard`): rated cards get a restrained `box-shadow` glow tinted by `scoreColorAlpha(rating, 0.2)`; unrated cards get none — a 9.5 reads warmer than a 4.0 at a glance.
  - **Starred gleam** (`.card-gleam`): a translucent gold stripe sweeps across starred entry/restaurant cards. Three triggers: (1) once on mount (`@keyframes gleamSweep`, 1.5 s, 0.35 s delay); (2) on hover/desktop (`@keyframes gleamSweepHover`, same keyframes, different name forces browser restart); (3) periodically every ~9 s via `@keyframes gleamPeriodic` (9 s total cycle: 1.5 s sweep then 7.5 s idle, loops `infinite`). Per-card stagger is set via the `--gleam-periodic-delay` CSS custom property on the host element (derived from item ID using the golden-ratio formula `((id * 0.618) % 3) + 3` seconds), which the `::after` inherits. All three triggers are disabled under `prefers-reduced-motion: reduce`. Host needs `position: relative` + `overflow: hidden`.
  - **Best of the Month gleam** (`.card-gleam` on ReigningChampionCard): reuses the same sweeping gold gleam as starred cards — sweeps once on mount, again on hover (desktop), and periodically every ~9 s (mobile-friendly).
