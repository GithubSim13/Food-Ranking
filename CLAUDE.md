...
- **Visual polish (second pass)**: built on the animation utilities above, all CSS-variable-only (`color-mix(in srgb, var(--gold) …%, transparent)` for translucent gold):
  ...
  - **Starred gleam** (`.card-gleam`): a translucent gold stripe sweeps across starred entry/restaurant cards once on mount and again on hover (`@keyframes gleamSweep`); host needs `position: relative` + `overflow: hidden`.
  - **Best of the Month gleam** (`.card-gleam` on ReigningChampionCard): reuses the same sweeping gold gleam as starred cards (`@keyframes gleamSweep`) — sweeps once on mount and again on hover.
