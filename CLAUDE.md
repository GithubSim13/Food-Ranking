...
client/src/
  ...
  components/
    ...
    analytics/
      AnalyticsPage.tsx # /analytics — stats and insights page; sections: Rating Distribution donut chart (avg rating shown as center label in donut hole, var(--accent)), Starred Ratio donut chart (starred count shown as center label in donut hole, var(--gold)), Top Categories by Avg Rating (min 2 rated entries), Best Restaurants by Avg Rating (min 2 rated entries), Most Visited Stores (top 10 by entry count, Nx format), Most Logged Categories (top 10 by entry count, Nx format), Category Insights card (Best Spot per Category + Score Breakdown by Category merged into one Card with kicker "CATEGORY INSIGHTS"; shared single-line scrollable pill strip from breakdownCategories alphabetically sorted; pill strip: flexWrap nowrap, overflowX auto, gap 6px, padding 2px 0, hide-scrollbar class; left/right gradient fade overlays (var(--paper-2)→transparent, 32px wide, position absolute, opacity driven by catPillScroll state via scroll listener + ResizeObserver on pillScrollRef); active pill auto-scrolls into view on effectiveCategory change via pillRefsMap Map ref; effectiveCategory = activeCategory ?? defaultBreakdownCat; Best Spot sub-section shows restaurants with ≥2 entries in selected category sorted by avg rating desc; Score Breakdown sub-section shows three T/V/C horizontal bars with full spectrum gradient and 400ms width transition; sections separated by var(--line) hr; empty state per sub-section if data insufficient), Rating Trajectory movers (entries with 2+ reviews, filter: improved/declined/all), Logging Activity Calendar (GitHub-style heatmap of review dates; year picker pills inline with "Logging Activity" heading in flex justify-content space-between header row; 53×7 grid cells 11×11px with 2px gap, coloured via color-mix(in srgb, var(--accent) 25/55/100%, transparent) by review count 0/1/2–3/4+; month labels above grid using absolute positioning at col×13px; Mon/Wed/Fri day-of-week labels; hover tooltip position:fixed with date + review count; horizontally scrollable wrapper), Consistency vs Taste scatter (plain SVG, X=Consistency rating3, Y=Taste rating1; quadrant dividers at x=5/y=5 with corner labels: Reliable & Tasty / Tasty but Inconsistent / Consistent but Bland / Avoid; dots r=5 fill var(--accent)/var(--gold) for starred, hover r=7; fixed tooltip with food/restaurant/category/T+C/overall; click opens entry modal; ResizeObserver for responsive width; Card has minWidth: 0, container div has overflow: hidden), Price vs Rating scatter (plain SVG, X=Price ₱ with maxPrice rounded up to nearest 50, Y=Rating 0–10; same dot/hover/click pattern as Consistency vs Taste; uses latest review with both price and overallRating non-null; shows empty state if <3 data points; shares 1fr 1fr grid row with Consistency vs Taste; Card has minWidth: 0, container div has overflow: hidden), Long Time No See (entries with exactly 1 review and non-null date, sorted oldest-visit-first; paginated LTNS_PAGE_SIZE = 10, prev/next buttons, "Showing X–Y of Z" label, row # uses global index ltnsPage × LTNS_PAGE_SIZE + i + 1; table columns: #/Entry/Category/Restaurant/Visited/Days Ago; Days Ago coloured: <180d var(--ink-mute), 180–365d var(--gold), >365d var(--badge-never-again); "Xd" Space Mono; local date parsing split on '-' not new Date()), Rating by Country (sortable table grouped by Entry.flag, null flag = "🏠 Home" row; FlagImage + full country name from countryList.ts (fallback raw code); columns: #/Country/Entries/Avg Rating (scoreColor + inline proportional bar)/Best Entry (accent clickable span opens entry modal); default sort avg rating desc; sort cycles asc/desc per column), Underrated Gems (entries with latestRating ≥ 8.0 and reviews.length === 1; responsive 3/2/1 card grid matching Starred Picks; card: flag, food name truncated, restaurant muted, category pill accent-wash/accent-ink, rating large accent mono, "1 visit" badge bottom-left, tryAgain blue dot via EntryFlagBadges, hover border var(--accent) transition 150ms; click opens entry modal; paginated GEMS_PAGE_SIZE = 9, prev/next buttons, "Showing X–Y of Z" label), Starred Picks grid (paginated, STARRED_PAGE_SIZE = 9, prev/next buttons, "Showing X–Y of Z" label); all computed client-side from ['entries'] query
    entries/
