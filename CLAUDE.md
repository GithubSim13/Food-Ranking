...
| **🏆 Hall of Fame** | Top 5 entries by highest overallRating (latest review per entry); full-width upward podium — bars grow upward, `alignItems: 'flex-end'` row, purple color scheme; order left to right: 4, 2, 1, 3, 5; bar heights: #1=65%, #2=50%, #3=40%, #4=28%, #5=18%; label (flag + name + score) above each bar; inside bars 1–3: trophy emoji (🥇🥈🥉), rank number, category, restaurant, review count; inside bars 4–5: medal emoji + rank only; clicking any bar opens entry modal via background-location pattern; large backdrop watermark "🏆 Hall of Fame" with `.hall-title-fame` green glow |
| **💀 Hall of Shame** | Bottom 5 rated entries (min one review) sorted by overallRating asc; full-width downward podium directly below Hall of Fame — bars grow downward, `alignItems: 'flex-start'` row, orange color scheme; same 5-column order; bar depths mirror fame heights; label (flag + name + score) below each bar; inside bars 1–3: skull/grimace/nauseated emoji, rank number, category, restaurant, review count; clicking any bar opens entry modal; large in-flow watermark "💀 Hall of Shame" with `.hall-title-shame` red glow below bars |
