...
- [x] Visual polish (second pass) — `useCountUp` promoted to `utils.ts` and applied to Home stat numbers (scroll-triggered via `useInViewOnce`) + About callout; score-based `box-shadow` glow on rated EntryCards (`scoreColorAlpha`); sweeping gold gleam on starred entry/restaurant cards and the Best of the Month card (`.card-gleam`, `@keyframes gleamSweep`); hover-lift on Home stat cards; all new effects CSS-variable-only (`color-mix` for translucent gold) and `prefers-reduced-motion` aware
- [x] Animation polish fixes — About "foods logged" count was rendering `0` (bottom-of-page `useInViewOnce` observer never fired); switched it to run on mount. Replaced the Best of the Month spotlight/pulsing-border effect (`.champion-spotlight`, `spotlightDrift`/`championGlow` keyframes, removed) with the shared `.card-gleam` sweep. Removed the per-card `.anim-delay-*` stagger on RestaurantsPage (broken in the column-first CSS-columns masonry) — cards now animate together and remount on sort change via an `id-sortCol-sortDir` key.
- [x] Bug fix: QuickRatePage data loss — PUT /api/reviews/:id was overwriting notes/date/price/uncertainRating with null; fixed server-side to do partial merge on only fields present in request body
- [x] One-off data recovery: restoreNotes.ts script parsed entries.md and restored notes to 12 reviews wiped by the above bug; script since deleted
- [x] Bug fix: `.card-gleam` hover re-trigger — hover animation was silently no-oping because the browser won't restart an animation when the same name is re-applied; fixed by splitting into `@keyframes gleamSweep` (mount) and `@keyframes gleamSweepHover` (identical, different name forces restart)
- [x] Gleam periodic re-trigger — added `@keyframes gleamPeriodic` (9 s cycle: 1.5 s sweep + 7.5 s idle, infinite) so the gold sweep re-fires every ~9 s on mobile where hover doesn't exist; per-card stagger via `--gleam-periodic-delay` CSS custom property (golden-ratio formula on item ID); all three triggers (mount, hover, periodic) disabled under `prefers-reduced-motion`