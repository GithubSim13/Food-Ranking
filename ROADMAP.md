...
- [x] Restaurants page redesign — replaced sortable table with masonry-style card grid (CSS columns); cards show avatar, name, entry count, avg rating bar, notes (when present); edit/delete icons top-right; expandable entry list per card with click-through to entry detail; restaurant notes field (String?, nullable) added to schema via migration add_restaurant_notes