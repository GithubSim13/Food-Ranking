            ...
            {podiumOrder.map(({ entry: fEntry, rank: fRank, height: fHeight, barColor: fColor, scoreOpacity: fOpacity }, barIndex) => {
              ...
              return (
                <div key={fEntry.id} style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    <div style={{ textAlign: 'center', marginBottom: '0.5rem', padding: '0 0.25rem', width: '100%', opacity: shameExpanded ? 0 : 1, transform: shameExpanded ? 'translateY(-10px)' : 'translateY(0)', transition: contentTr }}>
                    ...
                    <div style={{ width: '100%', height: fHeight, background: `linear-gradient(to bottom, ${fColor}, transparent)`, borderRadius: '6px 6px 0 0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, overflow: 'hidden', padding: '0.5rem 0.25rem', transform: shameExpanded ? 'scaleY(0)' : 'scaleY(1)', transformOrigin: 'top', transition: barTr }}>
                      <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: fRank === 1 ? '2.2rem' : fRank === 2 ? '1.7rem' : fRank === 3 ? '1.35rem' : fRank === 4 ? '1.05rem' : '0.9rem', color: `rgba(255,255,255,${fOpacity})`, lineHeight: 1, opacity: shameExpanded ? 0 : 1, transform: shameExpanded ? 'translateY(-10px)' : 'translateY(0)', transition: contentTr }}>{fRank}</span>
                      {fRank <= 3 && <>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: `rgba(255,255,255,${fOpacity * 0.65})`, textAlign: 'center', lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', width: '100%', opacity: shameExpanded ? 0 : 1, transform: shameExpanded ? 'translateY(-10px)' : 'translateY(0)', transition: contentTr }}>{fEntry.category}</span>
                        <span style={{ fontFamily: 'Hanken Grotesk, sans-serif', fontSize: '10px', color: `rgba(255,255,255,${fOpacity * 0.8})`, textAlign: 'center', lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', width: '100%', opacity: shameExpanded ? 0 : 1, transform: shameExpanded ? 'translateY(-10px)' : 'translateY(0)', transition: contentTr }}>{fEntry.restaurant}</span>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: `rgba(255,255,255,${fOpacity * 0.55})`, textAlign: 'center', lineHeight: 1.3, opacity: shameExpanded ? 0 : 1, transform: shameExpanded ? 'translateY(-10px)' : 'translateY(0)', transition: contentTr }}>{fEntry.reviewCount} {fEntry.reviewCount === 1 ? 'review' : 'reviews'}</span>
                        {fEntry.quote && <span style={{ fontFamily: 'Hanken Grotesk, sans-serif', fontSize: '0.68rem', fontStyle: 'italic', color: 'rgba(255,255,255,0.55)', textAlign: 'center', lineHeight: 1.3, marginTop: '6px', padding: '0 6px', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', opacity: shameExpanded ? 0 : 1, transform: shameExpanded ? 'translateY(-10px)' : 'translateY(0)', transition: contentTr }}>"{fEntry.quote}"</span>}
                      </>}
                    </div>
                  </div>
                </div>
